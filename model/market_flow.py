...
def analyze_market_flow(market_info, whale_trades, rules=None, bankroll=None):
    ...
    return {
        ...
        "risks": llm.get("risks", ""),
        "market_direction_comment": comment,
        "prior_prob": round(pp, 3),
        "prior_source": prior["source"],
        "prior_fallback": prior["is_market_fallback"],
        "llm_p_yes": round(p_yes, 3),
        "sm_pct": round(sm_pct, 1) if sm_pct is not None else None,
        "blended_prob": round(blended, 3),
        "edge": round(ev, 4),
